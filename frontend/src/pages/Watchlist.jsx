...
export default function WatchlistPage() {
    ...
    if (!user) {
        return (
            <div className="pt-32 px-6 lg:px-10 min-h-screen text-white pb-16 flex flex-col items-center justify-center">
                <div className="w-24 h-24 mb-6 rounded-full bg-white/5 flex items-center justify-center border border-white/5">
                    <svg className="w-10 h-10 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                </div>
                <h2 className="text-2xl font-black uppercase tracking-widest mb-4">Authentication Required</h2>
                <p className="text-gray-400 text-sm font-medium mb-8 text-center max-w-md">You must be signed in to view and manage your personalized watchlist.</p>
                <Link to="/login">
                    <button className="px-10 py-4 bg-primary text-black rounded-full text-xs font-black uppercase tracking-[0.2em] shadow-lg shadow-primary/20 hover:scale-105 transition-all">Sign In Now</button>
                </Link>
            </div>
        );
    }
